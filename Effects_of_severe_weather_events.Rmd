...
```{r}
storm_economic <- storm %>% 
  gather(type_damage, cost, propdmg, cropdmg) %>% gather(type_exp, magnitude, propdmgexp, cropdmgexp)
```
...
effect_economic_start <- storm_economic %>% 
  ...
  mutate(magnitude = ifelse(is.na(value), magnitude, value), magnitude = coalesce(magnitude, "0") %>% as.integer()) %>% 
  select(-value)
