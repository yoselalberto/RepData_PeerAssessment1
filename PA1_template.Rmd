...
The mean of steps taken each day is `r  mean(steps_date$steps) %>% ceiling`, and the median is `r  median(steps_date$steps)`
...
The grand total of NA's in the dataset is `r number_na`.  
...
The mean of the steps taken per day of the imputed data is `r number_steps_day_imputed_mean`, 
and the median of the steps taken per day of the imputed data is `r number_steps_day_imputed_median`.  

The median of the imputed data is the same than the original, but the mean increased from 
`r  mean(steps_date$steps) %>% ceiling` to `r number_steps_day_imputed_median`.  
